...
cleos push action trackeosacc initiate '{"supplier":"supplier", "products":[{"name":rice,"quantity":"5","price":"200"},{"name":"maize","quantity":"20","price":"100"}],"buyer":"buyer", "timestamp":"2017-08-08T20:55:13"}' -p supplier
...
```sh
docker stop eosio_trackeos_container
```
...
│   │   └── trackeos
│   │       └── trackeos.cpp // the main smart contract
...
The UI, index.jsx, reads the notes data directly from nodeos using 'getTableRows()'. The smart contract, trackeos.cpp, stores these notes in the multi index table using 'emplace()'' and 'modify()'.
...
```sh
docker exec -it eosio_trackeos_container bash
```
...
The smart contract can be found at `eosio_docker/contracts/trackeos/trackeos.cpp`(host environment), you can edit this smart contract. You will then need to compile and deploy the contract to the blockchain.
...
```sh
./scripts/deploy_contract.sh trackeos Supplychainacc Supplychainwal $(cat Supplychain_wallet_password.txt)
```
...
```sh
docker run --rm --name eosio_trackeos_container \
